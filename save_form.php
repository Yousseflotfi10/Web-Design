<?php
	...
	$check = mysqli_query($db_connect, "SELECT 1 FROM Cardata");

	if (!$check) {
		$create_table = "CREATE TABLE Cardata (id INT NOT NULL AUTO_INCREMENT, username TEXT NOT NULL,".
		"email VARCHAR(100) NOT NULL, car TEXT NOT NULL, age INT(3), Brand TEXT NOT NULL,".
		"additionchoice TEXT, ExtraText VARCHAR(100), PRIMARY KEY (id));";

		$table_created = mysqli_query($db_connect, $create_table) or die("Unable to create table.");
		};


	$query = "INSERT INTO Cardata (username, email, car, age, Brand, additionchoice, ExtraText)".
